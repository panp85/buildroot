    ...
    def clear(self, force=False):
        ...
        if os.path.exists(self.base_path):
            if not force and not os.path.exists(marker_file):
                print('The directory %s does not appear to have been created by ScriptTest' % self.base_path, file=sys.stderr)
                print('The directory %s must be a scratch directory; it will be wiped after every test run' % self.base_path, file=sys.stderr)
                print('Please delete this directory manually', file=sys.stderr)
